...
from northwind_db_api.crud import customer as crud
from northwind_db_api.schemas import Customer

customers_crud_router = APIRouter()


@customers_crud_router.get("/customers/", response_model=List[Customer])
def read_customers(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.get_customers(db, skip=skip, limit=limit)
    return users


@customers_crud_router.get("/customer/{customer_id}", response_model=Customer)
...
@customers_crud_router.post("/customer/", response_model=Customer, status_code=201)
...
@customers_crud_router.patch("/customer/{customer_id}", response_model=Customer, status_code=201)
@customers_crud_router.put("/customer/{customer_id}", response_model=Customer, status_code=201)
...
@customers_crud_router.delete("/customer/{customer_id}", status_code=204)
