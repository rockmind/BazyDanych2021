...
@orders_crud_router.get("/orders/", response_model=List[Order])
def read_orders(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.get_orders(db, skip=skip, limit=limit)
    return users


@orders_crud_router.get("/orders/{order_id}", response_model=Order)
...
@orders_crud_router.post("/orders/", response_model=Order, status_code=201)
...
@orders_crud_router.patch("/orders/{order_id}", response_model=Order, status_code=201)
@orders_crud_router.put("/orders/{order_id}", response_model=Order, status_code=201)
...
@orders_crud_router.delete("/orders/{order_id}", status_code=204)
